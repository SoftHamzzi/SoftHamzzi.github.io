...
- **`std::shared_ptr`의 크기는 raw 포인터의 두 배다.** 내부적으로 자원을 가리키는 raw 포인터뿐만 아니라 자원의 참조 횟수를 가리키는 raw 포인터도 저장해야 하기 때문이다.
...
- **`std::unique_ptr`에서는** 삭제자의 타입이 스마트 포인터 타입의 일부다.
...
- **raw 포인터로 `std::shared_ptr` 생성자를 호출하면 제어 블록이 생성된다.** 이미 제어 블록이 있는 객체로부터 `std::shared_ptr`를 생성하고 싶다면, raw 포인터가 아니라 `std::shared_ptr`나 `std::weak_ptr`를 생성자의 인수로 지정하면 된다.

**⚠️ 하나의 raw 포인터로 여러 개의 `std::shared_ptr`를 생성하면:**

```cpp
auto pw = new Widget;                              // pw는 raw 포인터
...
- **raw 포인터 변수를 `std::shared_ptr` 생성자의 인수로 사용하지 말 것.** 대신 `new`의 결과를 직접 전달하라.
...
- `std::shared_ptr`의 역참조 비용은 raw 포인터의 역참조 비용보다 크지 않음
...
- **raw 포인터 타입의 변수로부터 `std::shared_ptr`를 생성하는 일은 피해야 한다.**
