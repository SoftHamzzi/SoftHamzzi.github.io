...
- 이동 생성이 복사 생성보다 빠르고, 이동 배정이 복사 배정보다 빠른 이유가 바로 이것이다.
...
- **고유 소유권 포인터(`std::unique_ptr`, `std::auto_ptr`)로부터 `std::shared_ptr` 객체를 생성하면 제어 블록이 생성된다.** 고유 소유권 포인터는 제어 블록을 사용하지 않으므로, 피지칭 객체에 대한 제어 블록이 이미 존재할 가능성은 전혀 없다.
...
- 할당은 본질적으로 무료 (피지칭 객체를 위한 메모리와 함께 할당)
- `std::shared_ptr`의 역참조 비용은 생 포인터의 역참조 비용보다 크지 않음
- 참조 횟수 조작을 요구하는 연산(복사 생성, 복사 배정, 객체 파괴)에는 원자적 연산 한두 개가 더 소비되나, 각각 하나의 기계어 명령에 대응
