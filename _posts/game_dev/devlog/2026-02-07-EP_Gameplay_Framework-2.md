---
title:  "[UE5] 익스트랙션 슈터 1-2. Enhanced Input으로 FPS 캐릭터 구현"
excerpt: "입력을 어디에 둘지, 그리고 공중 관성 조율"
...
기능 자체는 어렵지 않다. 이 글에서 실제로 판단한 건 두 가지다.

1. **InputAction 에셋을 누가 소유할지**: PlayerController냐 Pawn이냐
2. **공중에서 관성을 얼마나 남길지**: 익스트랙션 슈터의 이동 감각

---

## 입력 구조: InputAction을 PlayerController에 뒀다

<details markdown="1"><summary>입력 바인딩 코드 (접기/펼치기)</summary>
...
MappingContext 등록은 `AEPPlayerController::BeginPlay()`에서 `UEnhancedInputLocalPlayerSubsystem`에 한다.
...
**PlayerController를 고른 이유:** 이 게임은 플레이어가 조종하는 클래스가 하나다.
장비만 바뀌지 Pawn 종류가 바뀌지 않는다. 입력 정의를 캐릭터마다 들고 다닐 이유가 없고,
죽고 리스폰해도 입력 에셋 참조가 그대로 남는 쪽이 단순하다.

**되돌려야 할 조건:** 탈것이 생기거나 조작 체계가 다른 Pawn(드론, 포탑)이 생기면
이 결정은 뒤집어야 한다. 그때는 Lyra 방식이 맞다. 두 번째 Pawn이 없어서 지금은 그 구조를 미리 만들지 않았다.

---

## 이동: Pitch와 Roll을 버리는 게 핵심

<details markdown="1"><summary>Input_Move 코드 (접기/펼치기)</summary>
...
`FRotationMatrix(...).GetUnitAxis(EAxis::X)`는 각도를 계산하는 게 아니라, 회전 행렬을 만들어서
그 기저축(앞 방향)을 꺼내는 코드다. 각도는 이미 `ControlRotation.Yaw`로 주어져 있고,
여기서 하는 일은 각도를 방향 벡터로 바꾸는 것뿐이다.

진짜 중요한 건 `FRotator(0.0, Yaw, 0.0)`의 0 두 개다.
...
이 함수는 `ControlInputVector`에 `Dir * Scale`을 누적할 뿐이다. 컨트롤러 기준이 되는 건
바로 위에서 `ControlRotation`으로 `Dir`을 만들었기 때문이지, 함수 자체가 컨트롤러에 묶여 있어서가 아니다.
...
<details markdown="1"><summary>Input_Look 코드 (접기/펼치기)</summary>
...
시점 변경에는 Server RPC를 쓰지 않는다. 그럴 필요가 없다. `ControlRotation`은 클라 로컬에서
즉시 반영되고, 서버에는 이동 패킷에 실려서 간다. `FSavedMove_Character`가 `SavedControlRotation`을
필드로 들고 있기 때문이다.

> 이동 패킷에 이미 자리가 있는데, 왜 별도 RPC를 만드는가. 이 질문을 계속 들고 간다.
...
<details markdown="1"><summary>점프 바인딩 코드 (접기/펼치기)</summary>
...
**의도한 거다.** 지금은 누르고 있으면 계속 뛰는 쪽을 골랐다. 이 판단은 나중에 바뀔 수 있다.

### 공중 관성: 손을 떼도 속도가 안 줄었다

문제는 다른 데서 나왔다. 앞으로 한 번 입력하고 손을 떼도 속도가 그대로 유지됐다.
공중에서 무한 활강이 되는 셈이다.

<details markdown="1"><summary>관성 설정 코드 (접기/펼치기)</summary>
...
`CalcVelocity`는 가속도가 0일 때만 브레이킹을 건다. 즉 `BrakingDecelerationFalling`은
공중에서 입력을 놓았을 때만 속도를 깎는다. 입력을 유지하면 그대로 날아간다.

정확히 원하던 동작이다. 조작하면 관성이 살고, 놓으면 죽는다.

`FallingLateralFriction`이 기본값 0이라는 게 전제다. 여기에 값을 주면 입력 중에도 감속이 걸려서
공중 조작이 둔해진다. 그래서 건드리지 않았다.
...
> DeadZone은 입력 값을 자르는 모디파이어다. 캐릭터 속도와는 무관하다.
> 입력 파이프라인(하드웨어 축 → Modifier → Trigger → Value)의 앞쪽에 있고, 속도는 그 결과물이라 볼 수도 없다.

---

## 달리기: 매 프레임 Reliable RPC를 쐈다

<details markdown="1"><summary>달리기 코드 (접기/펼치기)</summary>
...
`Triggered`는 누르고 있는 동안 매 프레임 발화한다. 쉬프트를 누르고 있는 동안,
매 프레임 Reliable Server RPC를 쐈다는 뜻이다. 60fps면 초당 60회. Reliable이라
ACK와 재전송 부담까지 붙는다.

트리거만 `Started`/`Completed` 쌍으로 바꿔도 RPC 횟수는 줄어든다. 하지만 그건 증상 완화지
해결이 아니다. 이동 상태를 이동 패킷 밖으로 보내고 있다는 구조 자체가 문제다.
