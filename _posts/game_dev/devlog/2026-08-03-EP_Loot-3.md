...
> Lyra는 이 길을 쓰지 않고 **서버도 자기 라인 트레이스를 돌린다**(`LyraGameplayAbility_Interact.cpp:78-122`). 이유는 Lyra의 F키가 어빌리티 *활성화*가 아니라 이미 떠 있는 어빌리티의 *트리거*이기 때문이다. **우리는 F키가 곧 활성화라 파라미터가 자연스럽게 실린다.**
...
해결책은 "새 채널"이 아니라 **"기본 응답 `Ignore` + 상호작용 대상만 `Block`"**이다.
...
- 반대로 `ECR_Block`으로 만들면 위의 함정을 밟는다
- 사거리가 250cm라 실질 피해가 작고, **Lyra도 같은 방식이다**
- 막아야 할 때가 오면 `ECC_WorldStatic`을 한 번 더 트레이스해 가리는 방식이 맞다. **채널의 `DefaultResponse`를 뒤집는 게 아니다**
